// $ cargo rb serial-dma-loopback-pool
...
mod app {
    ...
    #[init(local = [rx_buf: [u8; BUF_SIZE] = [0; BUF_SIZE], tx_buf: [u8; BUF_SIZE] = [0; BUF_SIZE], memory: [u8; 64] = [0; 64]])]
    fn init(mut ctx: init::Context) -> (Shared, Local, init::Monotonics) {
        ...
        let mut command = P::alloc().unwrap().init(Default::default());
        postcard::to_slice_cobs(&COMMANDS[0], command.as_mut()).ok();
        send_command::spawn(command).ok();
        (
            Shared {
                handle: None,
                rx,
    ...
    #[task(shared = [tx])]
    fn send_command(ctx: send_command::Context, command: Box<P>) {
        defmt::info!("TX: {:?}", command.as_ref());
    ...
    fn on_tx_dma(ctx: on_tx_dma::Context) {
        ...
        *cmd = (*cmd + 1) % COMMANDS.len();
        let mut command = P::alloc().unwrap().init(Default::default());
        postcard::to_slice_cobs(&COMMANDS[*cmd], command.as_mut()).ok();
        send_command::spawn_after(Milliseconds(2_000_u32), command).ok();
    }
    ...
    fn on_rx_dma(ctx: on_rx_dma::Context) {
        ...
        let data = P::alloc().unwrap().init(unsafe {
    ...
    fn on_idle(ctx: on_idle::Context) {
        ...
        let mut data = P::alloc().unwrap().init(Default::default());
        unsafe {
    ...
    fn parser(ctx: parser::Context, data: Box<P>) {
        match ctx.local.cobs_buf.feed::<Command>(data.as_ref()) {
